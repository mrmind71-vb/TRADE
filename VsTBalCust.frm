...
cField11 = myiif(cWhere & turn(cWhere, " And ") & " (TYPE = 'T')", "[SAL] - [PAY]") & " AS T_CASH0 "
...
'                           0                           1
    cString = "  select FILE3_10.CODE AS CODE , FILE3_10.DESCA AS DESCA , " & _
                ...
                cField8 & " , " & cField9 & " , " & cField10 & " , " & cField11 & " , " & cField12 & " ,' ' AS N12,' ' AS N13  " & _
...
Sub FIXGRID()
    ...
    .TextMatrix(0, 12 + 1) = "‘Ìﬂ«  €Ì— „Õ’·…"
    .TextMatrix(0, 13 + 1) = " «—ÌŒ √Œ— „»Ì⁄« "
...
ChqTable.Open cStrChq, con, adOpenStatic, adLockReadOnly, adCmdText
    ...
    For I = 1 To .Rows - 1
        ...
        If Not SalesTable.EOF Then
            .TextMatrix(I, 6) = mRound(SalesTable!T_COST)
            .TextMatrix(I, 7) = mRound(Val(.TextMatrix(I, 5)) - Val(.TextMatrix(I, 6)))
            .TextMatrix(I, 13 + 1) = Format(SalesTable!LASTSALES, "DD-MM-YYYY")
        End If
        ChqTable.Find " CODE1 = " & MyParn(.TextMatrix(I, 0)), , adSearchForward, adBookmarkFirst
        If Not ChqTable.EOF Then
            .TextMatrix(I, 12 + 1) = mRound(ChqTable!T_VALUE)
        End If
        For nCol = 2 To .Cols - 3
            .TextMatrix(I, nCol) = mRound(.TextMatrix(I, nCol))
        Next nCol
    Next I
